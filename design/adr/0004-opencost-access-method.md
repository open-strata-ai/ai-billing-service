# ADR-0004: OpenCost access method
...
Whether to check the Prometheus/OpenCost API directly or consume its exports needs to be aligned with the `ai-observability` namespace (§9.2).
...
This ADR solidifies the "OpenCost access method" into an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §16 open issue and is still open.
