...
GPU billing actually occurs with self-hosted inference (full file) (§8.1 D5 / §14.4). The advanced file GPU quota does not actually take effect. It needs to be clarified whether billing will be included.
...
This ADR solidifies "GPU billing timing" as an architectural decision record and incorporates it into `design/adr/` for continuous tracking. This issue stems from the `design/DESIGN.md` §16 open issue and is still open.
